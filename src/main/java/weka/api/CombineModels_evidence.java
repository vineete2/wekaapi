...
public class CombineModels_evidence {
...
/*
	...
	*/
        ...
        Instances evidencedata=  new Instances(new BufferedReader(new FileReader("data/eval/Test_Mix2.arff")));
        evidencedata.setClassIndex(evidencedata.numAttributes()-1);
        evidencedata.deleteStringAttributes();


		Classifier evidenceclassifier = (Classifier) weka.core.SerializationHelper.read("data/model/evidence/J48_Model.model");
        //System.out.println(test.instance(0));
        double[] prediction=evidenceclassifier.distributionForInstance(evidencedata.instance(1));
        System.out.println(prediction[0]);

		//	stacker.setMetaClassifier(new J48());
		stacker.setMetaClassifier(evidenceclassifier);//needs one meta-model
		...
		stacker.buildClassifier(evidencedata);

		//double postSMO[] = stacker.distributionForInstance();
		double[] result=stacker.distributionForInstance(evidencedata.instance(1));
